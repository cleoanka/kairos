...
ANALYST_NODE_SPECS: dict[str, AnalystNodeSpec] = {
    ...
    "microstructure": AnalystNodeSpec(
        # System-1 analyst (Kairos bridge): reads the causal perception bus
        # (LOB-Core) instead of a data vendor. Opt-in — not in the default
        # analyst set — so a pure-reasoning run is byte-for-byte unaffected.
        key="microstructure",
